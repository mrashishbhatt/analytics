...
mean(df$X); mean(df$Y)  #; used for simultaneous calculation
sum(df$X); sum(df$Y)
sd(df$X) ; var(df$Y)
...
plot(y=df$Y, x=df$X,xlab='Area in sqft', ylab='Sales Amount', type='p', ylim=c(0, max(df$Y)), main='Plot of Area Vs Sales', xlim=c(0,max(df$X)), col='blue',pch=24)

...
#date in yyyy-mm-dd
dates1a = c('2018-21-4', '2018-29-4') #anything in a single code is a string
class(dates1a)
?as.Date
...
class(dates1b)
dates1b
dates1b + 5     # + whenever we use, it helps in going forward in terms of dates.
as.Date('2018-12-26') + 5

#default format %Y-%m-%d

dates2a = c('1994-11-4')
class(dates2a)  # character so far
(dates2b = as.Date(dates2a))  # as.Date is used to convert normal strings to date formats
...
format(as.Date('4-11-1994', '%d-%m-%y'), '%A')
#System Date
Sys.Date()
...
dob=as.Date('29-06-1992', '%d-%m-%Y')
dob
Sys.Date()
(duration1 = Sys.Date() - dob)
as.numeric((Sys.Date() - dob)/365)


dob1=dob+ seq(1,1000,30)
dob1
length((dob1))
mean(as.numeric((Sys.Date()-dob1)/365))
x=as.numeric((Sys.Date()-dob1)/365)
boxplot(x)
...
(date1=seq(as.Date("2027-1-1"), as.Date("2018-1-1"), by="-6 month"))
...
seq(as.Date("2017/1/1"), by = "month", along.with = c(3,2,43))  # no significance of these numerical values
